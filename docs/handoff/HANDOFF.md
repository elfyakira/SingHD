...
- [ ] **診断結果の組み合わせタイプ（上位職）システム構築**:
  - 現状: 結果画面にセカンダリータイプの画像がない
  - 理想: プライマリー×セカンダリーの組み合わせで「上位職」を表示する
  - 例: 戦士×僧侶＝パラディン、勇者×賢者＝魔法剣士、など
  - 上位職ごとに専用画像を生成して表示
  - MBTI診断を参考に、各組み合わせタイプの詳細プロフィールを設計:
    - 「あなたには〇〇の素質があります」という伝え方
    - その素質でどういうことができるのか
    - どういう将来が待っているのか（キャリアビジョン・可能性）
  - ユーザーにとって「自分を知れた」「自分の可能性が見えた」という価値を提供しつつ、「この会社で活かせる」→ コンバージョンにつなげる導線設計
