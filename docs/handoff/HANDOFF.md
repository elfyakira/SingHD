...
**最終更新: 2026-04-04（セッション18 — Vercelデプロイ修正 + .vercelignore追加）**

---

## 現在のサイト状況

起業家志望者向けサイトとして大幅リニューアル済み。MIRAIKUページをTRIGGER風インタビューサイトとして再構築完了。アニメーション導入途中。カツヤクLP（組織活性化プロジェクト）を新規作成・デプロイ済み。MIRAIKU採用専用サイトをページ構成リニューアル（14ページ→7ページ体制）。感情導線の4段階に基づく構成。要件定義原文への文言復元完了。**TOPページを冒険の地図UIとしてカスタムレイアウトで再構築完了。** 各ページに画像素材配置済み。**冒険診断の上位職システム・カード型リザルト画面を実装。** **ヘッダーメニュー改修完了（出発の広場・冒険の知恵袋）。冒険者ガイドのコンテンツ量産パイプライン構築完了。** **冒険者ガイド初期3記事を公開（why-challenge / first-weapon / party-building）。SEO/LLMOコンテンツ戦略監査機能を構築。** **クライアントFB対応：ヘッダー「Sing RECRUIT」化、キャラクター公式プロフィール追加、募集要項に職種追加・キャリアパス・応募資格追加。** **冒険者ガイド追加3記事を公開（failure-is-exp / level-up-habits / leader-or-hero）。全6記事体制に。** **挑戦者ストーリーに屋宜勝正・清水駿之介を追加（全4名体制）。TOPページの挑戦者ストーリーを無限ループスライダー化。** **挑戦者ストーリー4ページのプロフィールカード改修完了（モバイル/PC高さ統一・uppercase修正）。** **メインサイト：Sing発スタートアップ企業セクションのSing.nexTロゴサイズを他3社と統一。** **`.vercelignore`追加（sing/・sing-next/除外）— Vercelデプロイエラー解消。**

---

## セッション18（04-04）で実施した作業：Vercelデプロイ修正

### .vercelignore追加

リポジトリ直下に別プロジェクト（`sing/`、`sing-next/`）が存在し、Vercelデプロイ時にsing-next内の`@/lib/site`が解決できずTypeScriptエラーでビルド失敗していた。

- **対応**: `.vercelignore` を作成し `sing/` と `sing-next/` を除外
- **結果**: 全35ページ正常ビルド、本番（https://jp-sing.com）にデプロイ完了
- セッション17で追加済みだったSing.nexTのフッターリンク・企業紹介ページが本番に反映
...
## セッション17（03-31）で実施した作業：Sing.nexTロゴサイズ調整

### Sing発スタートアップ企業セクション — ロゴサイズ統一

`src/app/project/page.tsx` のグループ会社セクションで、株式会社Sing.nexTのロゴが他3社（Sing・フライトップ・ゆめスタ）より小さく表示されていた問題を修正。

- **原因**: ロゴ画像（`singnextlogo.jpg`）内の余白が大きく、共通の`max-w-[200px]`制約では実際のロゴ部分が小さく見えていた
- **対応**: Sing.nexTのみ `max-w-[320px] scale-150 origin-left -ml-6` を適用し、視覚的に他3社と統一
- 他3社のロゴ表示は変更なし（`max-w-[200px]`）
...
- [x] ~~siteUrl修正~~ → jp-sing.com に移行済み（2026-03-21 ドメイン移行完了）
